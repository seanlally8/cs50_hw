...
def main():
    # Global assignment of lists: database
    # (for csv file), strcount (for holding values for repetitions of longest str sequence), tmplist (for comparing values in csv with strcount values)
    database = []
    strcount = []
    tmplist = []
...
def sequence(key, dna):
    ...
    for i in range(len(reps)):
        count = 0
        tmp = reps[i]
        for j in range(tmp, len(dna), len(key)):
